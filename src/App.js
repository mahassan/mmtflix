...
function App() {
  ...
  useEffect(()=>{
    const post = async() => {
        const res = await fetch('https://api.themoviedb.org/3/search/movie?api_key=d8c2552f21a3c339b4bc8cd51606624c&query='+searchTerm);
        const data =await res.json();
        setPageNumber(data.page)
        const {results} = data;
       setLength(results.length)
    ...
    <div className="App">
      <Header result={length} pageCount={pageNumber} setSearchTerm={setSearchTerm}  />
       ...
       {movies.length > 0 && movies.map((data)=>{
        return(
          <Library key={data.id} {...data} searchTerm={searchTerm} />
        )
       })}
    </div>
    </div>
